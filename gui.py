...
class BlancPrint:
    ...
    def format_text(self):
        ...
        text=text+"_______________________________________________________________________________________\r\n"
        #86 Char Max @ 11 mono font
	text=text+"Model".ljust(16)[:16]+" "
	text=text+"Manufacturer".ljust(16)[:16]+" "
	...
	text=text+"\r\n"
        ...
        if len(self.category.contents)>0:

	    	for i in self.category.contents:
	    		if (not i) or i.is_category():
	    			continue
    		...
    		text=text+"Category is empty ..."
    	
        self.pangolayout.set_text(unicode(text, "latin-1"))
       	desc = pango.FontDescription("mono 11")
...
class glade_window():
	...
	def handle_print(self, widget):
		BlancPrint(self.category_selected)
	...
	def export_category(self, filename, category):
		self.csv.save(filename, category.contents)
	...
	def import_category(self, filename ,category):
		...
		for row in dat:
			print row
			data.item(parent=ncat, model=row[0], manufacturer=row[1], description=row[2], quantity=row[3], cost=row[4], location=row[5])
			
		self.add_category_parent( category, ncat)
		self.category_select(ncat)
	...
	def category_select(self, category):
		...
		for i in self.category_selected.contents:
			if i.is_category():
				continue
			#Add to content
